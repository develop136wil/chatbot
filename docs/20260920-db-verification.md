...
커밋/푸시/운영 배포는 이 보고서 작성 시점에는 수행하지 않았습니다.
...
1. 테스트용 브랜치 커밋·푸시 승인 후 Regression Tests 실행.
2. native PostgreSQL 17에서 동시 요청 검증:
   - 서로 다른 24개 요청, 전체 한도 7 → 승인 정확히 7.
   - 동일 사용자 24개 요청, 개인 한도 3 → 승인 정확히 3.
   - 5개 사용자 30개 요청, 전체 9·개인 2 → 승인 정확히 9, 사용자별 최대 2.
3. CI 통과 후 실제 프로젝트 확인 → 운영 SQL 적용 → 운영 배포 및 실사용 확인.

단일 연결 SQL 통과는 동시 접속 안정성, Supabase 운영 설정, Python 3.11 의존성,
새 답변 품질·지연 검증을 대체하지 않습니다.

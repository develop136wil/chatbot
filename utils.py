...
def summarize_content_with_llm(context: str, original_question: str, chat_history: list[dict] = []) -> str:
    ...
    **결과물은 반드시 '{target_lang}'(으)로 작성해야 합니다.**
    ...
    2. **생략 규칙 (중요):** 원본에 없는 정보는 아예 항목을 만들지 마세요. "정보 없음"이라고 적지 마세요.
    3. **제외 대상:** '비용 부담', '주의사항', '참고' 섹션은 정말 치명적인 정보가 아니면 **과감히 생략**하여 깔끔하게 만드세요.
    4. **시각적 단순화:** 이모지(💵, ⛔️)를 절대 사용하지 마세요. 화면이 지저분해집니다.
...
def rerank_search_results(question: str, candidates: list) -> list:
    ...
    2. **'검사/진단' 질문 시 (가장 중요):**
       - 사용자가 '발달 검사', '장애 진단'을 찾고 있다면, 제목에 **'검사', '진단', '선별'**이 있거나 **'검사비'**가 포함된 문서가 1순위입니다.
       - **주의:** '지원'이라는 단어 하나만 보고 높은 점수를 주지 마세요. '검사 지원'이 아니라면 순위를 낮추세요.
       - **(감점 대상):** '창동종합사회복지관' 같은 특정 복지관의 '포괄적 지원사업'이나 '단순 상담'은 '검사'가 아니므로 후순위로 내리세요.
       - 예: "온라인 발달검사" (O), "발달장애인 정밀검사비 지원" (O) vs "미취학 아동지원사업" (X)
